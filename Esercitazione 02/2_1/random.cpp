...
double Random :: Parabola(void){ // accept-reject: y=pi/2*(1-x^2)
	double x = Rannyu();
	double y = Rannyu(0, M_PI/2.);
	while(y > (M_PI/2.*(1-pow(x,2))) ){
	  x = Rannyu();
		y = Rannyu(0, M_PI/2.);
	}
	return x;	
...
double Random::Bad_sampling(void){ // inversa cumulativa: y=3x^2
	return pow(Rannyu(), 1./3.);
}
