...
//variabili per g(r_i)
	sd = box/2.; // estremo destro della binnatura
	bin_size = sd/(double)nbins; // larghezza di ogni bin
  bin = new double[nbins]; // estremo sinistro di ogni bin
	dV = new double[nbins]; // elemento di volume per normalizzazione
  for(int i=0; i<nbins; i++){
		bin[i] = i*bin_size;
		dV[i] = (4*M_PI/3.)*( pow(bin[i]+bin_size,3) - pow(bin[i],3)) ;
  }
...
//Prepare arrays for measurements
  ...
  ik = 2; //Kinetic energy
  ie = 3; //Total energy
	iw = 4; //Virial ---> for pressure
	ig = new int[nbins];
	for(int i=0; i<nbins; i++) ig[i]=5+i; //g(r_i)
...
// correzioni di coda: 
	// vtail indica v_tail/npart
	// ptail indica w_tail/(3*epsilon*npart)
  vtail = (8.*M_PI*rho)/(3.) * (1./(3.*pow(rcut,9)) - (1./pow(rcut,3)));
...
//Print initial values for measured properties
  ...
  cout << "Initial total energy     = " << walker[ie]/(double)npart << endl;
	cout << "Initial pressure         = " << rho*walker[it] + (walker[iw]+ptail*3*npart)/(3*vol) << endl;
...
void Measure() //Properties measurement
{
  ...
  double dx, dy, dz, dr;
	...
	for(int i=0; i<nbins; i++) walker[ig[i]]=0;
...
// distance i-j in pbc
      ...
      dr = sqrt(dr);
			...
			if(dr<sd){
				walker[ig[(int)floor(dr/bin_size)]] += 2.;
				}
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Epot, Ekin, Etot, Temp, Press, Gstime, Gfinal;
   const int wd=12;
  ...
  cout << "Acceptance rate " << accepted/attempted << endl << endl;
    ...
    Etot.open("output_etot.dat",ios::app);
	  ...
	  Gstime.open("output_gstime.dat",ios::app);
    ...
    err_temp=Error(glob_av[it],glob_av2[it],iblk);
	  ...
	  stima_press = (blk_av[iw]/blk_norm + ptail*3*npart)/(3*vol) + rho*stima_temp; //Pressure
    glob_av[iw] += stima_press;
    glob_av2[iw] += stima_press*stima_press;
    err_press=Error(glob_av[iw],glob_av2[iw],iblk);

	  for(int i=0; i<nbins; i++){
			stima_g = (blk_av[ig[i]]/blk_norm)/(dV[i]*rho*npart);
			Gstime << setw(wd) << i << setw(wd) << (bin[i]+bin_size/2.) << setw(wd) << stima_g << endl;
			glob_av[ig[i]] += stima_g;
      glob_av2[ig[i]] += stima_g*stima_g;
		}
...
//Radial distribution function (final)
		if(iblk==nblk){
			for(int i=0; i<nbins; i++){
				err_g = Error(glob_av[ig[i]],glob_av2[ig[i]],iblk);
				Gfinal << setw(wd) << (bin[i]+bin_size/2.) << setw(wd) << glob_av[ig[i]]/(double)iblk << setw(wd) << err_g << endl;
			}
		}
